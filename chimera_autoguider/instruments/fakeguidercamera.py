...
class FakeGuiderCamera (CameraBase, FilterWheelBase):

    '''
    This FakeCamera is just like the main Fake instrument in chimera with the exception that it offsets the images
    by a random value on x and y in order to simulate telescope drift.
    '''
                ...
                if dome.isSlitOpen() and self["use_dss"]:
                    ...
                    if (abs(domeAZ-telAZ) <= 3):
                        ...
                        if telescope.getDec().D < -25:
                            ...
                            query_args["h"] = (ccd_height+self["max_offset"]) / 59.5  # ~1"/pix (~60 pix/arcmin) is the plate scale of DSS POSS2-Red
                            query_args["w"] = (ccd_width+self["max_offset"]) / 59.5
                        else:
                            query_args["v"] = "poss1_red"
                            query_args["h"] = (ccd_height+self["max_offset"]) / 35.3 # 1.7"/pix (35.3 pix/arcmin) is the plate scale of DSS POSS1-Red
                            query_args["w"] = (ccd_width+self["max_offset"]) / 35.3
